...
if [ ! -d /tmp/suitcase ]; then
    mkdir /tmp/suitcase
else
    rm -rf /tmp/suitcase/*
fi
TEMPDIR=/tmp/suitcase
#TEMPDIR=`mktemp -d /tmp/tmp.XXXXXXXXXX`

echo ec2-bundle-image -c $EC2_CERT -k $EC2_PRIVATE_KEY -u $ACCOUNTNUM -i $IMAGE_FILE -d $TEMPDIR -p ${IMAGE_NAME} -r x86_64

ec2-bundle-image -c $EC2_CERT -k $EC2_PRIVATE_KEY -u $ACCOUNTNUM -i $IMAGE_FILE -d $TEMPDIR -p ${IMAGE_NAME} -r x86_64
echo ec2-upload-bundle --retry -b $BUCKET -a $AWS_ACCESS_KEY -s $AWS_SECRET_KEY -m ${TEMPDIR}/${IMAGE_NAME}.manifest.xml
